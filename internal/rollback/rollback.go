...
func preflightStep(app gateway.AppPlan, usedIDs map[string]int) Step {
	return Step{
		...
		Readiness:    rollbackReadiness(app.Readiness),
		Evidence:     cutoverStepEvidence(app),
	}
}
...
func routeFromCutover(route gateway.Route) Route {
	return Route{
		...
		Readiness:   rollbackReadiness(route.Readiness),
	}
}
...
func cutoverStepEvidence(app gateway.AppPlan) []string {
...
func rollbackReadiness(readiness preparer.Readiness) preparer.Readiness {
	switch readiness {
