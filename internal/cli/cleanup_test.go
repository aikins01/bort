...
func TestRunCleanupInventoriesLeftoversAndSafeMetadata(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/project.one":
			...
			_ = json.NewEncoder(w).Encode(dokploy.Project{ProjectID: "p-proxy", Name: "proxy", Compose: []dokploy.ProjectComposeChild{{ComposeID: "c-proxy", Name: "proxy"}}})
		case r.Method == http.MethodGet && r.URL.Path == "/api/domain.byComposeId":
			if r.URL.Query().Get("composeId") != "c-proxy" {
				t.Fatalf("unexpected domain query: %s", r.URL.RawQuery)
			}
			_ = json.NewEncoder(w).Encode([]dokploy.Domain{})
		default:
			http.NotFound(w, r)
		}
	}))
	...
	for _, want := range []string{
		...
		"[present] proxy (p-proxy): safe metadata cleanup candidate: zero domains visible",
